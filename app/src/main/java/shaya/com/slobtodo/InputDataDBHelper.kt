...
class InputDataDBHelper(var mContext: Context?,var version:Int) : SQLiteOpenHelper(mContext, "INPUT_DATA", null, version) {
    companion object {
        val CREATE_INPUT_DATA = "create table input_data ( _id integer primary key autoincrement, title text not null,alarm integer not null,alarmTime integer not null, created_time text not null, updated_time text not null);"
