...
class InputDataDBHelper(var mContext: Context?,var version:Int) : SQLiteOpenHelper(mContext, "INPUT_DATA", null, version) {
    companion object {
        val CREATE_INPUT_DATA = "create table input_data ( _id integer primary key autoincrement, title text not null,lineId integer not null );"
        val CREATE_LINE_DATA = "create table line_data ( _id integer primary key autoincrement, ischecked boolean not null,todo text not null );"
